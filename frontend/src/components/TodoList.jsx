...
export const TodoList = () => {
  ...
  const fetchTodos = useCallback(async () => {
    try {
      const response = await axios.get("/api/v1/todos/all-todos");
  ...
  const handleDeleteTodo = async (todoId) => {
    try {
      await axios.delete(`/api/v1/todos/delete-todos/${todoId}`);
      fetchTodos();
    } catch (error) {
  ...
  const handleEditTodo = async (todoId) => {
    try {
      ...
      await axios.put(`/api/v1/todos/update-todos/${todoId}`, {
        text: editTodo,
      });
      fetchTodos();
  ...
  const handleCheckboxChange = async (todoId) => {
    try {
      await axios.patch(`/api/v1/todos/${todoId}/toggle`);
      fetchTodos();
    } catch (error) {
